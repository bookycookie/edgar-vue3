<template>
	<div class="container mb-5">
		<h1>Scripted questions</h1>
		<br />
		<b>
			Scripted questions enable teacher to append custom scoring (grading) code/script at the end of the scoring
			pipeline!
		</b>
		<br />
		<br />
		...
		<ol>
			<li>
				Edgar performs the built-in default scoring and forwards all the relevant data further down the pipeline
			</li>
			<li>
		...
		<ul>
			<li>
				...
				<ul>
					<li>
						All relevant data is embedded in the script object and is available simply as this.something, no
						parsing or evaluating necessary
					</li>
					<li>
						Teacher must implement only the getScore() method which must return the (potentially altered)
						score.
					</li>
				</ul>
			</li>
			<li>
				Other programming languages (Java, C, …) scripts are being sent to the code-runner, ie to the Judge0
				code execution engine – the very same used to evaluate students’ code. This means that Edgar invokes
				HTTP request to code-runner, which in turn passes the code to Judge0, where the code is compiled and
				executed in the sandbox
				<ul>
					<li>
						Relevant data is serialized as JSON and sent to the script as the single stdin argument. The
						script/program must deserialize the input in order to do anything useful.
					</li>
					<li>
						Teacher must return new score again, serialized as JSON, by writing that string to stdout. Edgar
						subsequently deserialized the stdout and proceeds.
					</li>
				</ul>
			</li>
		</ul>
		...
		<br />
		For instance, this is a valid JS script which simply appends to the hint (which students see in their feedback):
		<pre>
{
    getScore() {
		...
		>
		Let’s describe the process via an example. Say we want the student to print the string “hello” to the stdout in
		C without using the printf function; solution being eg to use the puts function. We create a C programming
		question and provide a single test-case which determines whether the “hello” is printed out. However, this only
		takes care of that part, now we will write the JS script to check whether “printf” occurs anywhere in the
		student’s code:
		<pre>
{
    getScore() {
		...
		>
		Note that it is OK to change the existing object – you don’t have to create new one (example of both approaches
		is above).
		<br />
		...
		<br />
		The structure varies with regards to the question type, however the score and record fields will always be set:
		<pre>
{
		...
		<pre>
{
    ...
    "gradingModel": {  // grading model assigned by teacher in test def
      ...
      "unanswered_score": 0
		...
		<br />
		<b>
			As with templates, the teacher can now damage the scoring process and so it is important to be careful and
			extensively test your scripts.
		</b>
		<hr />
		At last, note that templates combined with scripts make a very powerful combination!
		<br />
		Just as an example, one could make a free text question, and use a script to parse and score the submitted free
		text. For instance, we could randomly pick a float number, ask a student to convert it to eg IEEE754 (or binary,
		or hex, or whatever) and then programmatically determine the answer in the data object, which we finally check
		and score in the script. Hundreds of variations from a single question!
	</div>
</template>
