<template>
	<div class="container-fluid">
		<Card>
			<template #title>Peer Assessment</template>
			<template #content>
				<Accordion multiple>
					<AccordionTab header="What is Peer Assessment?">
						"Peer assessment is an educational activity in which students judge the performance of their
						peers and it can take different forms depending on the characteristics of its implementation,
						the learners and the learning context.
						<br />
						Peer assessment can take different forms ranging from summative purposes (e.g., peer grading,
						peer evaluation) to formative purposes (e.g., peer feedback)."
						<br />
						--
            ...
            students' solutions.">
						<br />
						First, we need to define a test and an assignment (question). We will define a test with a
						single fixed free text question.
						<ul>
							<li>
								Go to:
								<b>Questions → Course structure: node graph and edit</b>
								and define a new "Peer assessment" node (e.g. under root node) if you already don't have
								it.
							</li>
							<li>
								Go to:
								...
								<br />
								If your course has "show_attachments" flag ON, you will be able to attach a number of
								files to the question (e.g. images, project files, etc.).
							</li>
							<li>
								Go to:
							...
							<li>
								...
								<ul>
									...
									<li>
										Add the question from the "Peer assessment" node to the exam questions. Set some
										grading model, eg. 1/0/0
									</li>
									<li>
										Set Max score to the grading model max score, eg. 1. Since we're using free text
										questions (which are always evaluated as "correct") student will be awarded that
										one point. If you do not want to award the student, set Score ignored to on.
									</li>
									<li>Adjust the Upload files number and file size limits (default is 0 and 2MB).</li>
									<li>
										Adjust Title, abbreviation, password and available from/to. You can set Async
										submit to either on or off.
									</li>
						...
						<br />
						When they submit the test, their solution will be "graded" as correct (and possibly
						score-ignored).
						<br />
						...
						<ul>
							<li>
								Multiple question peer assessment test have not been tested. For the time being, please
								use one-test-one-question scheme.
							</li>
							<li>
								If max runs is greater than 1 and student uploads file1, file2 in the first run and
								file3 and file 4 in the second, when bulk download is employed all those files will be
								merged to a single user folder (containing file1-4) and possibly overwriting each other
								if they have the same names (eg. if in second run there was also file1)
							</li>
						</ul>
					</AccordionTab>
					<AccordionTab header="Peer Assessment Phase #2 - Shuffle and assessment">
						...
						<ol>
							<li>
								Define a "master" PA test. This test will reference the phase 1 PA test and provide a
								"mold" for the generation of actaul exams (assessments) that the student will take.
							</li>
							<li>Generate acutal PA tests, and disseminate passwords to students.</li>
						</ol>
						<b>
							There is a hard rule here: generated PA tests MUST have ordinals immediatelly following the
							MASTER.
						</b>
						<b>
							E.g. if MASTER test has test_ordinal = 65 and you have 5 assessment questions, they will
							have ordinals 66-70.
						</b>
						...
						<br />
						Assessment questions might also vary - eg PA-Q1 calls for 5 assessment question, and PA-Q2 calls
						for 7 assessment questions.
						<br />
						It is a good idea to place those 5 and 7 questions in separate nodes.
						<br />
						Also, prepare the "Calibration" question(s), and place them in a separate node. Calibration
						questions are just wrappers for zip attachments (question text is ignored). Give the attachment
						a GUID name so that it is similar to the GUID names of students' zipped submissions. E.g. take
						this and change a few letters: e2ea0cb6-69ed-4a12-b061-bc5cc824e02a.zip
						<br />
						The same PA-Q1/Q2 principle applies for the calibration questions, you will probably want to
						place them in separate nodes.
						<br />
						<br />
						Then:
						<ul>
							...
							<li>
								The upper half is standard - a test with FIXED questions. Add prepared questions, set
								password and timespans. You probably also want async=OFF.
								<br />
								For the time being, validation of PA#2 MASTER tests is disabled, so be careful.
								<br />
								Note: number of questions is ignored. Different PA1-Q1/Q2 groups may have different
								number of question and so this master number is ignored.
								<br />
								You CAN use grading model 0/0/0 and max_score 0 (as this kind of test will always be
								100% "correct").
								<br />
								Save.
							</li>
							<li>
								...
								<ul>
									...
									<li>
										set the number of calibration assignments and the node from which they will be
										randomly selected
									</li>
									<li>Save.</li>
									<li>
										...
										<ul>
											...
											<li>
												Side note: Fetch button fetches the questions from the selected nodes
												that are NOT used in the PA2 test, so you might want to press Fetch
												again after you add the question to narrow the list.
											</li>
							...
							<li>
								...
								<ul>
									<li>
										Hit Shuffle (keep groups) or Shuffle (all together) button, you can do it N
										times. The shuffle is visualized. Once you're happy:
										<ul>
											<li>
												Keep groups keeps students binned to their PA1-question group. You
												probably want this.
											</li>
											<li>
												All together shuffles them all together, and so the PA-Q1 student might
												get a PA-Q2 student's answer to assess.
											</li>
										</ul>
									</li>
									<li>
									...
									<li>
										...
										<br />
										Only "virgin" exams will be regenerated (from the master). You can access them
										via Exam definitions but they should not be changed individually.
										<br />
