<template>
	<div class="container-fluid">
		<Card>
			<template #title>Question templates</template>
			<template #content>
				Any question can be a templated question (mind the switch in the new question screen). Templates enable
				teachers to
				...
				as the templating engine, please consult the guide for more information. But, in a nutshell, handlebars
				replaces the expressions in double curly braces with values, for instance:
				<hr />
...
<p>1 + 2</p>
				...
				<br />
				Let’s walk through the entire process using an example: say we want to create a multiple-choice question
				asking the student to add two numbers (eg. “Calculate 2+3?”). Of course, we’ll make the numbers
				dynamically generated i.e. we will pick numbers by random.
				<br />
				So, we first must define the data object which will hold the randomly generated numbers x and y and
				answers a1..a4. Similar to runtime constraints for code questions, Edgar provides a hierarchy of objects
				which are used to construct the final data object:
				<ul>
				...
				This hierarchy facilitates the data object construction as the global and course data object (will)
				contain useful methods which the teacher can use in the question data object to initialize the object.
				<br />
				(their actual values/content can be seen in the Edit question form, in the log section of Data Object)
				<br />
				The final data object is constructed by merging all those objects and invoking the init() method on the
				final object.
				<br />
				In our example:
				<table>
...
d) I don’t know
				...
				These dynamic templated questions are generated when the student runs the exam and requests the question
				for the first time. The logic is as follows:

				<ul>
					<li>Get the question from the database</li>
					<li>If the question is NOT templated return the question to the client</li>
					<li>
						Otherwise, check if there is a generated and saved question (Edgar stores both HTML and data
						object)
						<ul>
				...
				<em>
					The question form has facilites to try the data object generation and template renedering. Still, a
					teacher is now in a postion to corrupt the question generation, so be careful - with great power
